...
// Helper to create a safe filename from a project path
const getProjectStoreKey = (projectPath: string) => {
  // Use btoa for a simple, safe transformation of the path to a filename-compatible string
  try {
    return `project-${btoa(projectPath)}.json`;
  } catch (e) {
    // Fallback for environments where btoa might not be available (very unlikely in Tauri)
...
const getInitialState = (): Omit<ProjectState, "rootPath" | "isInitialized" | "init"> => ({
    // Workspace
    fileTree: null,
    activeFilePath: null,
    selectedFilePaths: [],
    ...
    // Actions will be defined in the store creation
    setFileTree: () => {},
    setActiveFilePath: () => {},
...
export const useProjectStore = create<ProjectState>((set, get) => {
  ...
  return {
    ...
    init: async (rootPath: string) => {
        ...
        set({ ...savedState, rootPath, isInitialized: true });
    ...
    startReview: async (initialChanges) => {
        ...
        set({ changes, isReviewing: true, activeChangeId: changes.find(c => c.status === "pending")?.id ?? changes[0]?.id ?? null, errors: {}, lastReview: null, sessionBaseBackupId });
    },
    endReview: () => {
        ...
        set((state) => ({ isReviewing: false, lastReview: wasAnythingApplied ? { changes: state.changes, sessionBaseBackupId: state.sessionBaseBackupId } : null, changes: [], activeChangeId: null, sessionBaseBackupId: null, errors: {} }));
    },
    reenterReview: () => {
        set((state) => {
            if (!state.lastReview) return state;
            const { changes, sessionBaseBackupId } = state.lastReview;
            return { isReviewing: true, changes, sessionBaseBackupId, activeChangeId: changes.find(c => c.status !== "identical")?.id ?? changes[0]?.id ?? null, lastReview: null };
        });
    },
    ...
    applyChange: async (id) => {
        ...
        try {
            ...
            set((state) => ({ changes: state.changes.map((c) => c.id === id ? { ...c, status: "applied" } : c) }));
    ...
    revertChange: async (id) => {
        ...
        try {
            ...
            set((state) => ({ changes: state.changes.map((c) => c.id === id ? { ...c, status: "pending" } : c) }));
